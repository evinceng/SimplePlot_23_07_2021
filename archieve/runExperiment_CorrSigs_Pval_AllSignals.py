# -*- coding: utf-8 -*-
"""
Created on Mon Sep 13 14:06:44 2021
...
sensorID = 1
signalName = 'BVP'
sensors = {1:['empatica', {'ACC':['AccX', 'AccY', 'AccZ'], 'BVP':['BVP'], 'EDA':['EDA']}]}
...
for sensorID, sensor in sensors.items():
    for sensorFileNameExt, signalList in sensor[1].items():
        for signalName in signalList:
            print(sensorFileNameExt)
            print(signalName)           
            for cuID in uIDs: 
                users[cuID][sensorID] = {}
                users[cuID][sensorID][sensorFileNameExt] = {}
                users[cuID][sensorID][sensorFileNameExt][signalName] = {}
...
for sensorID, sensor in sensors.items():
    for sensorFileNameExt, signalList in sensor[1].items():
        for signalName in signalList:
            ...
            for cuID in uIDs:
                signal_x_df = getSignalDf(cuID, sensorID, signalName, sensorFileNameExt)
